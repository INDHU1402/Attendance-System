...
def delete():
    ...
    file_name = os.path.join('/home/indhu/Desktop/AttendanceSystem/ss6/', secure_filename(fn))
    os.remove(file_name) 
    folder = 'ss6'
    images = []
...
def update():
    ...
    file_name = os.path.join('/home/indhu/Desktop/AttendanceSystem/ss6/', secure_filename(file.filename))
    file.save(file_name)
    fn1 = os.path.join('/home/indhu/Desktop/AttendanceSystem/ss6/', secure_filename(fn))
    os.remove(fn1)
    folder = 'ss6'
    images = []
    for filename in os.listdir(folder):
...
def admin():
    ...
    for admin in adminsList:
        ...
        if admin[0] == nm and psw == ps:
            ...
            for filename in os.listdir(folder):
                images.append(filename)
            return render_template('admin.html', images=images, flg=True)
...
def face_recognition():
    ...
    with open('Attendance.csv','r+') as f:
        ...
        for line in myDataList:
            entry = line.split(',')
            nameList.append(entry[0])
    pth = "url_for('static', filename=" + "ss6/" + nameList[-1] + ".jpg')"
